...
/**
 * @file multiclass_batch.cpp
 * @brief a tools which can test batch-images for multiclass
...
static void show_result(int                                      image_idx,
                        ...
                        const std::vector<std::vector<float> >   result_data,
                        char*                                    log_buf,
                        int&                                     log_buf_len,
                        long long int                            forward_cost)
{
    ...
    {
        for(int j = 0 ; j < (int)(result_data[i].size()); j ++)
        {
            valid_count ++;
            cur_log_len += sprintf(cur_log + cur_log_len, "%.6f, ", result_data[i][j]);
        }
    }
    if(valid_count > 0)
    {
...
static int forward_image(vision::Multiclass*     multiclass_interface,
                         int                     image_idx, 
...
{   
    ...
    res = multiclass_interface->run(raw_image, output);
    if(0 != res)
    {
...
int main(int argc, char* argv[]){
    ...
    if(0 == image_list.size())
        goto DLCV_MULTICLASS_BATCH_LEAVE;
    ...
    {
        cur_forward_cost  = 0;
        int cur_res       = forward_image(multiclass,
                                          i, 
                                          img_file_dir.c_str(),
                                          image_list[i].c_str(),
                                          log_buf + log_offset,
