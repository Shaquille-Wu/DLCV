...
int main(int argc, char* argv[]) {
    ...
    std::string jpatch = "[{\"op\":\"replace\", \"path\":\"/postprocess/ops/0/param/conf_thresh\", \"value\": 0.3}]";
    IModelRunner* det = new IModelRunner(cfgfile, jpatch);

    vision::DLCVOut output;
    std::map<std::string, std::vector<Box> > boxes_map;
    HighClock clk;
    clk.Start();
    int ret = det->run(image, output);
    clk.Stop();
    std::cout << "detection run time:" << clk.GetTime() / 1000 << " ms." << std::endl;
    delete det;

    // test for detection model, which should has_boxes
    if(ret != 0 || !output.has_boxes) {
        std::cout << "Detector run err:" << ret << std::endl;
        return -1;
    }
    boxes_map = output.boxes_map;
